    ...
    return (
      <section>
        <ul className='HomePage-ul'>
          {allPosts.map(post => (
            <li className='HomePage-li' key={`post-${post.id}`}>
              <Link to={`/post/${post.slug}`} className='HomePage-link'>
        ...
        <div className='HomePage-showMoreWrapper'>
          {areMorePosts
            ? <button className='HomePage-button' onClick={() => loadMorePosts()}>
